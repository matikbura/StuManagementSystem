...
<div id="addContent">
    <form action="${pageContext.request.contextPath}\UpdateScoreServlet" method="post">
        <table style="border-collapse: collapse;margin: 0 auto; ">
            <tr>
                <td class="td1">成绩id</td>
                <td class="td2">
                    <input disabled="disabled" type="text" value="${requestScope.score.id}"/>
                    <input type="hidden" name="id" value="${requestScope.score.id}"/>
                </td>
            </tr>
            <tr>
                ...
                <td class="td2">
                    ...
                    <select name="studentNo">
                        ...
                        <c:forEach items="${requestScope.studentList}" var="studentList">
                            <c:choose>
                                <c:when test="${requestScope.score.student.studentNo==studentList.studentNo}">
                                    <option value="${studentList.studentNo}" selected>${studentList.name}</option>
                                </c:when>
                                <c:otherwise>
                    ...
                    <label>
                        <select name="arrangeNo">
                            ...
                            <c:forEach var="courseArrangeList" items="${requestScope.courseArrangeList}">
                                <c:choose>
                                    <c:when test="${courseArrangeList.arrangeNo==requestScope.score.courseArrange.arrangeNo}">
                                        <option value="${courseArrangeList.arrangeNo}" selected>${courseArrangeList.arrangeNo}</option>
                                    </c:when>
                                    <c:otherwise>
            ...
            <tr>
                <td class="td1">成绩：</td>
                <c:if test="${requestScope.score!=null}">
                    <td class="td2"><input type="text" size="3" name="score" value="${requestScope.score.score}"/></td>
                </c:if>
                <c:if test="${requestScope.score==null}">
