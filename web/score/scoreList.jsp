        ...
        <form  action="${pageContext.request.contextPath}/CheckScoreServlet" method="post">
            ...
            <div class="queryinput">
                ...
                <select name="classNo">
                    ...
                    <c:forEach items="${requestScope.classtblList}" var="classtblList">
                        <c:choose>
                            <c:when test="${classtblList.classNo==requestScope.classNo}">
                                <option value="${classtblList.classNo}" selected>${classtblList.className} </option>
                            </c:when>
                            <c:otherwise>
                                <option value="${classtblList.classNo}">${classtblList.className} </option>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
                </select>
            ...
            <div class="queryinput">
                学生学号：<input type="text" name="studentNo" value="${requestScope.studentNo}" size="10"/>
            </div>
            <div class="queryinput">
                学生姓名：<input type="text" name="name" value="${requestScope.name}" size="5"/>
            </div>
            <div class="queryinput">
        ...
        <table style="margin: 0 auto;margin-top: 20px;border-collapse: collapse;">
            ...
            <c:forEach var="listAllScore" items="${requestScope.list}" >
                <tr>
                    ...
                    <td>${listAllScore.courseArrange.classtbl.className}</td>
                    <td>${listAllScore.courseArrange.teacher.teacherName}</td>
                    <td>${listAllScore.courseArrange.studyRoom}</td>
                    <td>${listAllScore.student.studentNo}</td>
                    <td>${listAllScore.student.name}</td>
                    ...
                    <td>
                        <a href="ShowScoreServlet?id=${listAllScore.id}">查看</a>
                        <a href="DeleteScoreServlet?id=${listAllScore.id}">删除</a>
                    </td>
                </tr>
            </c:forEach>
        </table>
        <a href="PreAddScoreServlet" style="margin: 30px 0 10px 630px;" id="addButton2">新增</a>
        <div id="pageButton">
            <c:if test="${requestScope.curPage>1}">
                <c:if test="${requestScope.courseNo!=null||requestScope.classNo!=null||requestScope.teacherNo!=null||requestScope.teacherName!=null||requestScope.studentNo!=null||requestScope.name!=null||requestScope.scoreMin!=null||requestScope.scoreMax!=null}">
